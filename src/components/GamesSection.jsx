  ...
  const applyFilters = () => {
    return games
      .filter(game => {
        ...
        if (selectedCategory !== "all" && !game.genres.includes(selectedCategory)) return false;
        if (game.price > price) return false;
        const activePlatforms = Object.keys(platforms).filter(k => platforms[k]);
        if (!game.platforms.some(p => activePlatforms.includes(p.toLowerCase()))) return false;
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-between p-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  ...
                  <div className="mb-2 flex items-center gap-3">
                    {game.discount > 0 ? (
                      <>
                        <span className="line-through text-gray-400 text-sm">R$ {game.originalPrice.toFixed(2)}</span>
                        <span className="text-green-400 font-bold text-lg">R$ {game.price.toFixed(2)}</span>
                      </>
                    ) : (
                      <span className="text-green-400 font-bold text-lg">R$ {game.price.toFixed(2)}</span>
                    )}
            ...
            >
              ...
              <div className="flex justify-between items-center p-6 border-t border-gray-700">
                <div className="flex flex-row gap-4">
                  {previewGame.discount > 0 && <span className="line-through text-gray-400">R$ {previewGame.originalPrice.toFixed(2)}</span>}
